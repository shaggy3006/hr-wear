  ...
  if (!product) {
    return (
      <div className="max-w-lg mx-auto px-6 py-16 text-center">
        <p className="text-hr-hint text-sm">Produit introuvable.</p>
        <Link href="/catalogue" className="mt-4 inline-block text-xs text-hr-dark underline">
          Retour au catalogue
        </Link>
      </div>
    );
            ...
            <>
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="flex items-center border border-hr-border rounded overflow-hidden">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-9 h-9 flex items-center justify-center text-hr-muted hover:bg-hr-warm">
                    <Minus size={13} />
                  </button>
                  <span className="w-10 text-center text-sm font-medium text-hr-dark">{qty}</span>
                  <button onClick={() => setQty(Math.min(product.stock, qty + 1))} className="w-9 h-9 flex items-center justify-center text-hr-muted hover:bg-hr-warm">
                    <Plus size={13} />
                  </button>
                </div>
              ...
              <button onClick={handleAdd} className="flex items-center justify-center gap-3 w-full bg-hr-dark text-white text-xs tracking-widest py-3.5 rounded-sm hover:bg-hr-brown mb-3">
                <ShoppingBag size={15} />
                {added ? "AJOUTÉ !" : "AJOUTER AU PANIER"}
              </button>

              <Link href="/commande" className="flex items-center justify-center w-full border border-hr-border text-hr-dark text-xs tracking-widest py-3.5 rounded-sm hover:bg-hr-warm">
...
export default function ProduitPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-xs text-hr-hint tracking-widest">CHARGEMENT...</div>}>
      <ProductContent />
    </Suspense>
  );
}
